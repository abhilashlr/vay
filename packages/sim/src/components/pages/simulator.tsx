...
import { FleetServiceApi, simulatableVehicles } from "../../data/fleet-service.api";
import { simulatorData} from '../../data/heavy-duty';

export const SimulatorPage: FC = () => {
  const [currentVehicleId, setCurrentVehicleId] = useState(simulatableVehicles[0].vin)
  ...
  const simulatePositionOfVehicle = (vehicleId: string) => {
    ...
    updaterIntervalPoolRef.current[vehicleId] = setInterval(() => {
      FleetServiceApi.updatePositionOfVehicle(vehicleId, idx).then(({ done }) => {
        if (!done) {
          idx = idx + 10;
        } else {
  ...
  return (
    <form>
      <fieldset>
        ...
        <textarea value={JSON.stringify(simulatorData)} rows={5} cols={100} readOnly />
        <p className="text-muted">Coming soon - Upload your own JSON or GPX track and simulate!</p>
      </fieldset>
      <fieldset>
        ...
        <button type="button" onClick={() => simulatePositionOfVehicle(currentVehicleId)} disabled={!startSimulationMutation.isSuccess}>Update Vehicle position</button>
      </fieldset>

      <fieldset>
      <h2>Simulate all vehicles</h2>
        <button type="button" onClick={simulateAllVehicles} disabled={!startSimulationMutation.isSuccess}>Simulate all vehicles</button>
      </fieldset>
    </form>
  )
}